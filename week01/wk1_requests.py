...
# myurl = 'https://maoyan.com/films/1375'

response = requests.get(myurl,headers=header)
if response.status_code == 200:
    print(response.text) 
print("状态码为%d"%(response.status_code))

bs_info = bs(response.text, 'html.parser')

def get_url_type(myurl_type):
    response = requests.get(myurl_type,headers=header)
    # print(response.text)
...
for tags in bs_info.find_all('div', attrs={'class': 'movie-item-info'}):
    time.sleep(2)
    #电影名称
    filmname =tags.find('a',).text
    print(filmname)
    #上映时间
    releasetime =tags.find('p',attrs={'class':'releasetime'}).text
    print(releasetime)

    #电影类型  https://maoyan.com/films/1375        
    filmtype = get_url_type( 'https://maoyan.com' + tags.find('a',).get('href'))
    print(filmtype)
